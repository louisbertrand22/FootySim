...
async def test_simulate_match(session):
    ...
    session.add(season); await session.flush()
    ...
    for i in range(11):
        session.add(Player(club_id=c1.id, name=f"A{i}", age=20+i, position="MF", pace=60, shot=60, pass_=60, defend=60))
        session.add(Player(club_id=c2.id, name=f"B{i}", age=20+i, position="MF", pace=60, shot=60, pass_=60, defend=60))
    await session.flush()

    fx = Fixture(season_id=season.id, round=1, date="2024-08-01", home_club_id=c1.id, away_club_id=c2.id)
    session.add(fx); await session.commit()
