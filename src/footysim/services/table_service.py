...
async def build_table(session: AsyncSession, season_id: int) -> dict[int, TableRow]:
    ...
    res = await session.execute(
        select(Match, Fixture).join(Fixture, Fixture.id == Match.fixture_id).where(Fixture.season_id == season_id)
    )
    for match, fixture in res.all():
        home = rows.setdefault(fixture.home_club_id, TableRow(club_id=fixture.home_club_id))
        away = rows.setdefault(fixture.away_club_id, TableRow(club_id=fixture.away_club_id))
